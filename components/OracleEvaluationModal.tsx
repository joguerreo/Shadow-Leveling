...
const OracleEvaluationModal: React.FC<OracleEvaluationModalProps> = ({ player, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="relative w-full max-w-lg bg-surface-dark border-2 border-indigo-500/50 rounded-3xl shadow-2xl overflow-hidden my-8 animate-modal p-6 space-y-6 system-glow">
        <div className="flex items-center justify-between pb-3 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-xl bg-indigo-950 border border-indigo-500 flex items-center justify-center text-indigo-400">
            ...
            <div>
              <h3 className="text-white text-lg font-black uppercase italic font-display">
                Oráculo del Sistema (NVIDIA AI)
              </h3>
              <p className="text-slate-400 text-xs">Evaluación táctica de tus hábitos y debilidades</p>
            </div>
          </div>
          ...
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-slate-300 rounded-xl text-xs font-bold uppercase transition-all"
              >
                Nueva Consulta
              </button>
              <button
                onClick={onClose}
                className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-lg shadow-indigo-600/30 active:scale-95"
              >
                Entendido
              </button>
            </div>
